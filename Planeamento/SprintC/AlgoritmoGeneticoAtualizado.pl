...
% parameteriza��o
inicializa(Data):-write('Numero de novas Geracoes: '),read(NG), 			(retract(geracoes(_));true), asserta(geracoes(NG)),
...
gera(Data, Camiao, MelhorViagem,TempoViagem):-
	inicializa(Data),
	gera_populacao(Pop,Data,Camiao),
	avalia_populacao(Pop,PopAv,Data),
	...
	Estabilizacao is 3, 
	GeracoesIguais is 0,
	gera_geracao(0,NG,PopOrd,TempoExecucao,TempoMaximo,0,GeracoesIguais,Estabilizacao,MelhorViagem*TempoViagem),nl,nl.
...
avalia_populacao([],[],_):-!.
avalia_populacao([Ind|Resto],[Ind*V|Resto1], Data):-
	determinarTempo(Data,eTruck01, Ind, V),
	avalia_populacao(Resto,Resto1,Data).
