...
export default (app: Router) => {
	...
	route.get('/listUsers', (req, res, next) => ctrl.listUsers(req, res, next));
	...
	route.patch(
		...
		async (req: Request, res: Response, next: NextFunction) => {
			...
			try {
				const authServiceInstance = Container.get(AuthService);
				const userOrError = await authServiceInstance.cancelarUser(req.body as IUserDTO);

				if (userOrError.isFailure) {
					logger.debug(userOrError.errorValue());
					return res.status(403).send(userOrError.errorValue());
				}
				const { userDTO, token } = userOrError.getValue();
	...
	route.get('/email/:email', (req, res, next) => {
		ctrl.getUserByEmail(req, res, next);
	});
};
